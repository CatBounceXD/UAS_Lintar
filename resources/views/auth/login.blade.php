...
<head>
    ...
    <style>
        body { 
            background-color: #f3f2f1; /* Warna abu-abu khas background Microsoft */
...
<body>
    <div class="login-box">
        ...
        @if ($errors->any())
            <div class="error-msg">Alamat email atau sandi salah.</div>
        @endif

        <form method="POST" action="{{ route('login') }}">
            @csrf
            <div class="input-group">
                <input type="email" name="email" placeholder="Email, phone, or Skype" value="{{ old('email') }}" required autofocus>
            </div>
